...
{
    public:
    // it will find the position of arbitrary pointer 
    //TC == O(n^2)
    Node*find_arb(Node*curr1,Node*curr2,Node*destination)
    {
        if(destination==NULL)
        return NULL;
        //curr1==iterator on original list pointing to head
        //curr2==iterator on cloned list pointind to root
    ...
    Node *copyList(Node *head)
    {
        ...
        while(curr)
        {
            temp->arb=find_arb(head,root,curr->arb);
            temp=temp->next;
            curr=curr->next;
        }
        return root;
